	...
	public void create () {
		...
		for(Wall w : wallProp) {
			wall.add(modelBuilder.createBox(w.getDimension().x, w.getDimension().y, w.getDimension().z, 
					new Material(TextureAttribute.createDiffuse(new Texture("room.png"))), 
		...
		cam.lookAt(new Vector3(camPosition).add(snake.getVecF()));
		cam.update();
	...
	private void update() {
		if(System.currentTimeMillis() - gameT > 30) {
			...
			if(camCtrl != null) {
				...
				cam.position.set(camPosition);
				camCtrl.update();
				System.out.println(camCtrl.getCounter());
				if(camCtrl.getCounter()>=camCtrl.getStep()) {
					camCtrl = null;
					gameC = 0;
				}
			} else {
				camPosition.add(snake.getVecF().scl(.1f));
				cam.position.set(camPosition);
				cam.lookAt(new Vector3(camPosition).add(snake.getVecF()));
				cam.update();
			}
			...
			if(gameC%10 == 0 && camCtrl == null) {
				if(MyMath.cubeCollision(snake.getModelInstance().get(0).transform.getTranslation(new Vector3()), 
						new Vector3(5f, 5f, 5f), apple.getModelInstance().transform.getTranslation(new Vector3()), 
				...
				for(int i = 1; i < snake.getModelInstance().size(); i++) {
					if(MyMath.cubeCollision(snake.getModelInstance().get(0).transform.getTranslation(new Vector3()), 
						new Vector3(5f, 5f, 5f), snake.getModelInstance().get(i).transform.getTranslation(new Vector3()), 
						new Vector3(5f, 5f, 5f))) {
						snake.removeTail(i);
						break;
					}
				...
				for(Wall w : wallProp) {
					if(MyMath.cubeCollision(snake.getModelInstance().get(0).transform.getTranslation(new Vector3()).add(snake.getVecF()), 
						new Vector3(5f, 5f, 5f), w.getPosition(), w.getDimension())) {
						boolean goTop = true;
						for(Wall w2 : wallProp) {
							if(MyMath.cubeCollision(snake.getModelInstance().get(0).transform.getTranslation(new Vector3()).add(snake.getVecT()), 
									new Vector3(5f, 5f, 5f), w2.getPosition(), w2.getDimension())) {
								goTop = false;
								break;
							}
						}
